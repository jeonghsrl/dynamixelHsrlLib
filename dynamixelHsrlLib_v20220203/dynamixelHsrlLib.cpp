...
///////////////////////////////////////////////////////////
int32_t EPCM_Set_Init(uint8_t jnum, float dynAngAtJointHomeAng, float jointHomeAng, int32_t delayTime){ 
 ...
 //---速度・加速度プロファイル設定
 dxl_wb.itemWrite(DJ[jnum].id, "Profile_Velocity",20);  //リフトの速度制限を200*0.229 rpmに設定
 dxl_wb.itemWrite(DJ[jnum].id, "Profile_Acceleration",20);   
 result = dxl_wb.torqueOn(DJ[jnum].id, &log); 
...
// Serial.println(get_pulse); 
 result = dxl_wb.torqueOff(DJ[jnum].id, &log); 
 result = dxl_wb.itemWrite(DJ[jnum].id, "Homing_Offset",-get_pulse+jointHomeAng/DJ[jnum].pul2deg, &log); //関節座標系での角度を設定
...
//---------配列のtrajectorypointsを目標角度に順次セット
uint8_t exeTrajectoryPoints(uint8_t jnum){
     ...
     else{ DJ[jnum].trIdx=DJ[jnum].trIdxMax;  return 1;}
     }
